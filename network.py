...
class Net(nn.Module):
    def __init__(self, hyperparam):
        ...
        self.nospike = LI_no_Spike([hyperparam['n_outputs']-1], hyperparam = hyperparam)
...
class RSNNet(nn.Module):
    def __init__(self, hyperparam):
        ...
        self.fc4 = nn.Linear(hyperparam['neuron_count'], hyperparam['neuron_count'], bias = hyperparam['use_bias'])
        self.conv4 = nn.Conv1d(1, 1, kernel_size=hyperparam['conv_ker_size'], padding=hyperparam['conv_ker_size']//2, bias = hyperparam['use_bias'])
        self.frc4 = nn.Linear(hyperparam['neuron_count'], hyperparam['neuron_count'], bias = hyperparam['use_bias'])
        self.dr4 = nn.Dropout(p=hyperparam['dropout'])
        if(hyperparam['batchnorm'] == 'tdBN'):
            self.bn4 = tdBatchNorm0dSeq(hyperparam['neuron_count'], Vth = hyperparam['Vth'], hyperparam=hyperparam)
            if hyperparam.get('recurrent_batchnorm', False):
                self.bnr4 = tdBatchNorm0dSeq(hyperparam['neuron_count'], Vth = hyperparam['Vth'], hyperparam=hyperparam, alpha=0.5)
                self.bn4.alpha = 0.5
        self.sp4 = RLIFSpike([hyperparam['neuron_count']], hyperparam = hyperparam)
        

        self.fc5 = nn.Linear(hyperparam['neuron_count'], hyperparam['n_outputs'], bias = hyperparam['use_bias'])
    ...
    def forward(self, x):
        ...
        self.sp4.reset(batch_size = n_batches)
        ...
        spikeCount4 = torch.zeros(n_batches, self.fc4.weight.shape[0], device=x.device)
        ...
        for step in range(steps):
            ...
            # OPTIONAL MIDDLE LAYER
            if self.hyperparam['layers'] == 4:
                if self.hyperparam['inner_layer'] == 'fc': x_ = self.fc3(spikes2 + spikes1)
            ...
            if self.hyperparam['layers'] == 2: final_input = spikes1
            elif self.hyperparam['layers'] == 3: final_input = spikes2
            elif self.hyperparam['layers'] == 4: final_input = spikes3 + spikes2 + spikes1

            
            # SLOW LAYER
            if step % self.hyperparam['skipLen'] != 0:
                continue
            if self.hyperparam['inner_layer'] == 'fc': x_ = self.fc4(final_input)
            elif self.hyperparam['inner_layer'] == 'conv': x_ = self.conv4(final_input.unsqueeze(1)).squeeze(1)
            xr = self.frc4(self.sp4.sp)
            x_ = self.dr4(x_)
            if(self.hyperparam['batchnorm'] != 'none'):
                x_ = self.bn4(x_)
            spikes4 = self.sp4(x_ + xr)
            spikeCount4 += spikes4
        ...
        spikeCount2 /= steps
        spikeCount3 /= steps
        spikeCount4 = spikeCount4 / steps * self.hyperparam['skipLen'] 

        if self.hyperparam['layers'] == 2:
            return out,(spikeCount1,spikeCount4)
        elif self.hyperparam['layers'] == 3:
            return out,(spikeCount1,spikeCount2, spikeCount4)
        else: #self.hyperparam['layers'] == 4
            return out,(spikeCount1,spikeCount2, spikeCount3, spikeCount4)
    ...
    def constrain(self, hyperparam):
        if(hyperparam['constrain_method']=='eval'):
            with torch.no_grad():
                ...
                self.sp4.Vth.data = torch.clamp(self.sp4.Vth,min=0)
                self.sp4.tau.data = torch.clamp(self.sp4.tau,min=0, max=1)
